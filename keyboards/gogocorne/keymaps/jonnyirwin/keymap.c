/*
 * GoGoCorne keymap — 36-key split (LAYOUT_split_3x5_3).
...
/* BSPC: outer NAV thumb + SPC thumb (LH1 + LH0 in 36-key = MO(_NAV) + KC_SPC) */
const uint16_t PROGMEM bkspc[]   = {MO(_NAV),KC_SPC,   COMBO_END};
/* SYS: both inner NAV thumbs = MO(_NAV) + MO(_SYM) */
