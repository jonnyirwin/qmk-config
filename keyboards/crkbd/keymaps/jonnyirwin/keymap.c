/*
 * CRKBD keymap — 42-key split (LAYOUT_split_3x6_3).
 * Outer column: ESC/TAB on left, BSPC/ENT on right.
...
/* On CRKBD (42-key), the inner 5 columns per side match the 34/36-key position.
 * Modifier combos use the same letter keys — positions shift by +1 vs 30-key due to outer col. */
...
/* FN: K + / (inner and outer right bottom, same keys as 34/36-key) */
const uint16_t PROGMEM fn_lyr[]  = {KC_K,    KC_SLSH,  COMBO_END};
/* SYS: NAV thumb + SYM thumb */
const uint16_t PROGMEM sys_lyr[] = {MO(_NAV),MO(_SYM), COMBO_END};
/* BSPC: BSPC outer thumb + NAV thumb */
const uint16_t PROGMEM bkspc[]   = {KC_BSPC, MO(_NAV), COMBO_END};
/* TMUX: ENT thumb + SYM thumb */
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_CK]  = LAYOUT_split_3x6_3_wrapper(LAYER_CK_3x6_3),
    [_NAV] = LAYOUT_split_3x6_3_wrapper(LAYER_NAV_3x6_3),
    [_SYM] = LAYOUT_split_3x6_3_wrapper(LAYER_SYM_3x6_3),
    [_FN]  = LAYOUT_split_3x6_3_wrapper(LAYER_FN_3x6_3),
    [_SYS] = LAYOUT_split_3x6_3_wrapper(LAYER_SYS_3x6_3),
};
